...
| `BGE_M3_TRT_INBAND_JIT_GUARD` | `true` | When true (TRT EP only), refuses any chunk whose padded sequence length is `>= BGE_M3_TRT_INBAND_JIT_GUARD_SEQ` AND exceeds the pool's warmed engine coverage, returning `503` instead of issuing the `session.run()` that would trigger a pathological in-band TRT JIT (the process-killing failure mode). Set to `0` to restore the pre-guard behavior. See Key Gotchas. |
| `BGE_M3_TRT_INBAND_JIT_GUARD_SEQ` | `4096` | Sequence-length floor for the in-band JIT guard. Chunks below this length are never refused (a cold JIT there is bounded); only uncovered chunks at/above it are refused. Sits between the default warmup tiers `2048` and `8192`, so it is a no-op when `BGE_M3_MAX_SEQ_LENGTH` is below it. |
...
- **TRT cold-start: use `BGE_M3_WARMUP_ONLY=1`** — the default 24-shape grid takes ~135–270 min on first deploy. Run as an ECS init container to decouple compilation from service startup. Set `healthCheckGracePeriodSeconds ≥ 10800`.
...
- **TRT fatal engine build error → immediate worker exit** — `is_trt_engine_build_fatal` detects "failed to build engine" and "failed to create engine from network" patterns. When either fires during inference, the worker sends the error to the caller and returns `Err` from `run_worker`, causing `WorkerGuard` to decrement `live_workers`. ECS replaces the task once all workers exit. This is a harder signal than the circuit breaker: no retry, no reload — the CUDA driver state is considered unrecoverable. Detection uses `format!("{e:#}")` (the full anyhow source chain) because `run_worker` wraps embed errors with `.context(...)`; a plain `{e}` would see only the outermost context string.
- **In-band JIT guard prevents the crash before it happens** (`embedder/jit_guard.rs`, default ON for TRT): the `is_trt_engine_build_fatal` worker-exit and the circuit breaker are *reactive*: they only help if `session.run()` returns an `Err`. The worst TRT failure (an uncovered `seq=8192` `:both` shape whose autotuner requests 20 GiB-1 TiB scratch on a VRAM-saturated device) hard-kills the process via SIGSEGV / OOM-kill **before** any `Result` is returned, so nothing in Rust can catch it. The guard prevents it: a pool-wide `warmed_seq_ceiling` (`Arc<AtomicUsize>`, raised via `fetch_max` after prewarm / engine-propagation / adaptive compiles) records the max sequence tier *any* worker successfully warmed (engine plans live on shared EFS, so one worker's success covers the pool). Before each chunk's `session.run()`, `embed_{dense,sparse,both}` call `jit_guard::guard_chunks`; a chunk with `seq >= guard_seq && seq > warmed_seq_ceiling` is refused with `TrtJitRejection` → HTTP `503` (retriable) instead of crashing. This is the exact worker-3 failure mode: stride sharding gives worker 3 all `seq=8192` shapes; if they fail to compile, the ceiling stays at `2048` and `seq=8192` requests are refused on every worker rather than SIGSEGV-ing the process. Healthy steady state has zero false positives (ceiling = grid max). Guard refusals do NOT trip the circuit breaker (the worker is healthy). Self-heals: adaptive warmup / propagation raise the ceiling when a tier later compiles. Greppable WARN tag: `trt_jit_guard_refused`.
