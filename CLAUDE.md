...
- **Probe arena warm-up** — `run_probe` runs a `(1, 64)` `session.run()` BEFORE the actual probe sweep starts and discards the result. ORT lazily allocates its session arena on the first call, which appears as a ~1 GB constant offset on the first per-shape RSS delta and pollutes the OLS fit. The warm-up forces the arena to be initialised so subsequent per-shape `rss_delta` readings reflect only the incremental allocation attributable to that shape. Without the warm-up the cost-model fit degrades and the server falls back to conservative defaults that under-budget high-seq workspace by 5–10×.
