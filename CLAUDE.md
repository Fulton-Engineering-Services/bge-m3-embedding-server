...
- **TRT warmup grid MUST cover small batches (1 AND 2)** — On 2026-05-22, `lockbox-bge-m3-gpu-baseline` task `a81a2c44…` deployed with `BGE_M3_TRT_WARMUP_SHAPES=1x128,1x512,1x2048,1x8192` (batch=1 only). The first `/v1/embeddings:both` request that bin-packed to a batch=2 chunk triggered TRT JIT for an unseen shape; the autotuner requested **1,116,691,496,960 bytes (~1 TiB)** for one tactic on the fused `value/MatMul + LayerNorm` foreign-node, the CUDA allocator returned `Error Code 1`, and TRT emitted `failed to create engine from network`. `BGE_M3_TRT_MAX_WORKSPACE_BYTES` (set to 10 GiB) does NOT bound autotuner tactic scratch buffers — this is a TRT EP limitation, not a config issue. The default in-code grid is now `{1,2,4,8,16,32} × {128,512,2048,8192}` (24 shapes). When overriding `BGE_M3_TRT_WARMUP_SHAPES`, always include rows for batches 1 AND 2 — the server emits a `trt_warmup_shape_coverage_gap` WARN at startup if either is missing.
- **TRT fatal engine build error → immediate worker exit** — `is_trt_engine_build_fatal` detects "failed to build engine" and "failed to create engine from network" patterns. When either fires during inference, the worker sends the error to the caller and returns `Err` from `run_worker`, causing `WorkerGuard` to decrement `live_workers`. ECS replaces the task once all workers exit. This is a harder signal than the circuit breaker: no retry, no reload — the CUDA driver state is considered unrecoverable. **Production-proven 2026-05-22** (see `is_trt_engine_build_fatal` doc-comment for the full incident timeline).
...
- **`/health/deep` for ECS and ALB** — Point both ECS `healthCheck.command` and the ALB target group at `/health/deep` instead of `/health`. The deep check exercises the actual TRT session.run() path (batch=1, seq≈8) and returns 503 on inference failure, catching the 2026-05 silent-failure mode where `/health` returned 200 OK while all real embedding requests returned 500.
