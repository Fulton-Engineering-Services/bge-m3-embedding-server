...
- **TRT plans are compute-capability-specific:** TRT engine plan files embed the GPU compute capability (`sm_XX`), CUDA version, TRT version, and ONNX model SHA. An L4 compiles to `sm_89`; an A10G compiles to `sm_86`; a T4 compiles to `sm_75`. Plans compiled on one GPU family **cannot be used on another** — the TRT EP will silently recompile on a cache miss. EFS-mounted caches help only within a homogeneous ASG (all instances the same GPU family + same AMI). Mixed-GPU ASGs (e.g. T4 and A10G in the same group) will each pay the full cold-compile cost on their first task placement. Keep ASG instance families consistent when using a persistent engine cache.
