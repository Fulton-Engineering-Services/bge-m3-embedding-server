...
- **Parallel ORT session init contaminates per-worker RSS deltas** — `/proc/self/statm` reports process-wide RSS, so a worker that reads `post_load_rss` while other workers are mid-initialization sees the cumulative allocation of all concurrent loads. The root cause of the 2026-05-09 production incident (`probe_status=failed`, `per_worker_workspace=0`, bin-pack degraded to `batch=1`). Fix: `EmbedPool::spawn` loads workers **sequentially** (leader first, then followers one at a time) so each worker's RSS delta reflects only its own ORT session. Never change Phase 2 back to parallel spawning without re-establishing a measurement-clean alternative.
- **Median not max for aggregation** — `EmbedPool` stores the **median** of per-worker RSS deltas (not the max). Median is robust to a single outlier from page-cache settling or ORT arena jitter. The prior `fetch_max` approach would have returned the inflated outlier value, re-introducing the measurement contamination bug via a different code path. Do not revert to `fetch_max`.
...
- **ECS Managed Instances cgroup-v2 detection** — ECS Managed Instances (Bottlerocket) launch containers without `--cgroupns=private`, so `/sys/fs/cgroup/memory.max` resolves to the host unified-hierarchy root where the value is `"max"`. The actual container memory limit is set at a deeper path recorded in `/proc/self/cgroup` (unified format: `0::<path>`). `detect_available_memory()` now reads that file, extracts the cgroup path, and walks ancestors until it finds a numeric limit — falling through to host_ram only when the entire walk yields `"max"`. Root cause of the v0.15.0-rc4 production OOM: the server reported `source=host_ram, available_bytes=20.7 GiB` (host MemAvailable) instead of the 28 GiB container limit, causing the probe to think it had less budget than it actually had. The absolute-RSS guard (below) was still needed because the probe itself was accumulating ORT arena RSS past the cgroup ceiling.
- **Probe RSS-cap guard — ORT arena retention across probe shapes** — the startup probe sweeps `(batch, seq)` shapes via `session.run()`. ORT's memory arena retains pages between calls, so each successive shape adds to cumulative process RSS rather than releasing after the call. The guard in `run_probe` reads `current_rss` before each shape and skips the shape if `current_rss + 4 × estimated_per-call_cost > cgroup_limit × 87.5%`. The `4×` multiplier is empirically derived from the rc4 incident logs (measured `rss_delta` was 24–49× the conservative model prediction at mid-range shapes). This guard fires only on Linux where `/proc/self/statm` is readable. Root cause of the v0.15.0-rc4 OOM: probe swept `(1,4096)` when process RSS was already at 21 GiB (17 GiB model weights + prior probe shapes), pushing total past the 28 GiB cgroup ceiling.
- **Probe arena warm-up (rc6+)** — `run_probe` runs an arena warm-up `(1, 64)` `session.run()` BEFORE the actual probe sweep starts and discards the result. ORT lazily allocates its session arena on the first `session.run()`, which appears as a ~1 GB constant offset on the first per-shape RSS delta and pollutes the OLS fit (the rc5 fit returned `None` because the deltas were near-flat across all shapes — y was dominated by arena init, not workspace). The warm-up forces the arena to be initialised so subsequent per-shape `rss_delta` readings reflect only the incremental allocation attributable to that shape. Without the warm-up the cost-model fit is unrecoverable from the static probe shape set and the server falls back to conservative defaults that under-budget high-seq workspace by 5–10× — leading to OOM under real traffic even when the probe sweep itself completes safely.
- **Probe serialised before readiness checks (rc6 fixed permit-drop bug)** — the probe runs in a `tokio::spawn` task that holds all `cfg_workers` semaphore permits via an `OwnedSemaphorePermit`, blocking new `/v1/embeddings*` requests (they queue behind the semaphore; `/health` still returns 200). The dense/sparse readiness checks run inside the probe task after the probe completes, and `state.ready` is flipped only then. **rc5 regression that rc6 fixes:** rc5 used `acquire_many` and bound the permit to a local `_probe_lock` in the parent function, but `tokio::spawn` returns synchronously so the permit was dropped immediately, before the spawned task started — leaving `available_permits=cfg_workers-1` during the probe window and allowing real traffic to OOM the container. rc6 uses `Arc<Semaphore>::acquire_many_owned` and moves the permit into the spawned task closure so it survives across the probe + readiness work.
- **Probe shape set restored to 7 in rc6** — `(1, 4096)` and the dynamic `(1, max_seq)` shape are back in `PROBE_SHAPES`, made safe by three independent layers: (1) the arena warm-up handles lazy ORT init so RSS deltas at high seq are bounded, (2) the conservative `fits()` gate rejects shapes whose per-call workspace exceeds `rss_ceiling`, (3) the absolute-RSS guard rejects shapes whose projected total RSS would breach `cgroup_limit × 87.5%`. The dynamic `(1, max_seq)` shape doubles as a runtime quadratic anchor; if the model cannot run at `max_seq` the error path skips it (no fail-fast — failure surfaces as an ORT error on the first real request, more actionable than a startup OOM).
- **`Xenova FP16/INT8` long-context note (updated for rc6)** — `validate_max_seq_shape` runs at startup as a tokenizer + ndarray shape check (no `session.run()`); the actual `(1, max_seq)` `session.run()` runs as part of the probe sweep, gated by the RSS guard. If the Xenova model variant does not support the configured `BGE_M3_MAX_SEQ_LENGTH` the probe shape errors and is skipped — the failure surfaces at the first real embedding request. Use `BGE_M3_MODEL=fp32` or lower `BGE_M3_MAX_SEQ_LENGTH` if long-context inference fails.
