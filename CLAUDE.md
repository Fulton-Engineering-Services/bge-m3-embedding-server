...
- **rc7 production capacity at `max_seq=8192` on a 28 GiB cgroup task** — measured directly on the rc7 deployment (CloudWatch heartbeat at 23.8 GB process RSS, 4 workers): per-worker high-water at `(1, 8192)` is ~10.3 GB (1.4 GB model weights + 1.0 GB ORT arena baseline + 7.9 GB peak workspace). The cost-model fit fails at `max_seq=8192` because ORT switches attention kernels somewhere between seq=2048 (12 MB delta) and seq=4096 (1976 MB delta) — the two-coefficient quadratic model in `fit_cost_model` cannot fit that discontinuity, so the bin-packer falls back to conservative defaults that under-budget by ~12× (`chunk_cost(1, 8192) = 670 MB` vs measured 7.9 GB). Capacity table: `cfg_workers × 10.3 GB + OS_HEADROOM ≤ 28 GiB`. **Workers=2 is the safe cap on a 28 GiB cgroup at `max_seq=8192`** (~21 GB used, ~7 GB free). Workers=4 (rc7 deployment) stayed up because real `(1, 8192)` traffic is rare in codekeeper's workload but a single concurrent pair of long-document requests dispatched to two cold-arena workers would push process RSS past the cgroup ceiling. To run more workers without lowering `max_seq`, raise the task `memoryMiB` (e.g. workers=4 fits comfortably at 56 GiB; workers=5 fits tightly at 56 GiB with ~4 GB free).
