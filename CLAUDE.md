...
| `BGE_M3_MODEL` | `fp32` | Model variant: `fp32` = `BAAI/bge-m3` (~2.16 GB/session); `fp16` = `Xenova/bge-m3` (~1.08 GB/session); `int8` = `Xenova/bge-m3` quantized (~568 MB/session). **FP16 is not recommended**: ORT inserts `Cast(fp16→fp32)` nodes after every weight access; these fragment the CoreML execution graph, preventing GPU subgraph fusion — FP16 is 6–9× slower than FP32 with zero CoreML benefit. **INT8 + MLAS EP** is the recommended memory-saving option: near-FP32 latency (code_symbols +22%, tool_descriptions −9%) at 74% memory reduction. INT8 + CoreML EP is 42–79% slower than INT8 + MLAS for the same reason (DequantizeLinear nodes fragment the graph). INT8 dense cosine similarity vs FP32: mean=0.976, p5=0.969, min=0.963 — see `docs/performance.md`. |
