...
- **Dockerfile.cuda uses the `cu12` pyke.io prebuilt — NOT `nvrtx`.** This was the root cause of the 2026-05 codekeeper silent-persistence outage. The pyke CDN at `ort-sys-2.0.0-rc.12/build/download/dist.txt` publishes two GPU-relevant artifacts for `x86_64-unknown-linux-gnu`: `nvrtx` and `cu12`. Inspection of the `nvrtx` archive (extract → `ls`) shows it contains ONLY `libonnxruntime.a`, `libonnxruntime_providers_shared.so`, and `libonnxruntime_providers_nv_tensorrt_rtx.so` — the new `NvTensorRtRtxExecutionProvider` for RTX-class GPUs. It does NOT contain `libonnxruntime_providers_tensorrt.so`, the standard `TensorrtExecutionProvider` that `ort::ep::TensorRT` requires. With the standard TRT EP `.so` missing at runtime, ORT silently falls back to MLAS/CPU when our code calls `apply_execution_providers` (because `ExecutionProviderDispatch::error_on_failure` defaults to `false` and the `ort/tracing` feature was not previously enabled, swallowing the registration error). Outcome: `session.run()` keeps returning `Ok(_)` because CPU works, but no `.engine` files ever land in `{BGE_M3_CACHE_DIR}/trt-engines/`. The `cu12` artifact contains both `libonnxruntime_providers_tensorrt.so` and `libonnxruntime_providers_cuda.so` AND `libonnxruntime_providers_nv_tensorrt_rtx.so`, so it covers all three EPs we might want. `Dockerfile.cuda` now downloads and SHA-256-verifies the `cu12` prebuilt, sets `ORT_LIB_LOCATION=/opt/ort`, builds with `--features cuda,tensorrt`, and copies `libonnxruntime_providers_{shared,tensorrt,cuda}.so` into `/usr/local/bin/` in the runtime stage so ORT's `dlopen` finds them next to the binary (see "Static-linked ORT requires providers next to the binary" below). The runtime stage also installs `libnvinfer10`, `libnvinfer-plugin10`, `libnvonnxparsers10` from NVIDIA's CUDA APT repo because `libonnxruntime_providers_tensorrt.so` links to `libnvinfer.so.10`, `libnvinfer_plugin.so.10`, and `libnvonnxparser.so.10`. Do not add `download-ort` back — it is only for CI where `ORT_LIB_LOCATION` is unavailable.
