...
- **Probe shape set trimmed to 5** — `(1, 4096)` removed (ORT arena OOM risk at high seq) and the dynamic `(1, max_seq)` capability check removed (replaced by `validate_max_seq_shape`, a shape-only ndarray construction that does not call `session.run()`). The trimmed 5-shape set `[(1,64), (4,64), (1,256), (1,1024), (1,2048)]` recovers fitted `(a,b)` within 5% of ground truth — see `fit_cost_model_recovers_known_coefficients_from_7_probe_shapes` test. Long-context compatibility (`max_seq > 2048`) is validated at the ndarray level at startup; ORT-level compatibility surfaces as an error on the first real `/v1/embeddings` request.
- **Probe serialised before readiness checks** — in v0.15.0-rc5 the probe runs in a `tokio::spawn` task that holds all `cfg_workers` semaphore permits, blocking new `/v1/embeddings` requests (they queue behind the semaphore; `/health` still returns 200). The dense/sparse readiness checks run inside the probe task after the probe completes, and `state.ready` is flipped only then. This prevents concurrent inference (ALB health checks, queued requests) from contaminating the per-shape RSS deltas. The prior approach (readiness + probe running concurrently) caused the first probe shape `(1,64)` to report `rss_delta=3074 MiB` — 3× the actual per-call cost — because two concurrent `session.run()` calls both contributed to the process-wide RSS delta measurement.
- **`Xenova FP16/INT8` long-context note (updated)** — the startup probe no longer runs `session.run()` at `max_seq` during startup. If the Xenova model variant does not support the configured `BGE_M3_MAX_SEQ_LENGTH`, the error surfaces at the first real embedding request rather than at startup. Use `BGE_M3_MODEL=fp32` or lower `BGE_M3_MAX_SEQ_LENGTH` if long-context inference fails on the first request.
