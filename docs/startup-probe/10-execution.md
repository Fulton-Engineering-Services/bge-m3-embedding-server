...
> The probe takes ~120 s on a cache miss. Blocking startup on it would stall liveness probes and delay rolling-update completion. This page explains how the probe runs in the background, how it serializes with real traffic during the measurement window, and how the fitted coefficients are handed to running workers without locks.

## Intuition

A naïve startup looks like this: load the model, run the probe, fit the coefficients, then open traffic. Two minutes of startup latency. Liveness probes time out, the load balancer marks the task unhealthy, and the deployment fails. Bad.

A slightly better naïve startup runs the probe in a background task: load the model, *spawn* the probe, mark the server ready immediately. Faster startup, but now real `/v1/embeddings` traffic can hit a worker while the probe is reading process-wide RSS — and every concurrent allocation contaminates the measurement. The probe ends up fitting the wrong curve, the coefficients are garbage, the bin-packer over-budgets, and the next long-context request OOM-kills the worker. Worse than the naïve serial startup.

The right answer is a **traffic gate**: spawn the probe in the background, but block real `/v1/embeddings` traffic at a semaphore *until the probe finishes*. The server's `/health` endpoint can return 200 throughout, so liveness probes pass. The load balancer treats the task as healthy. Real traffic queues at the semaphore for the probe window (a few seconds to ~2 minutes), then floods through when the probe releases its hold.

Implementing this correctly turns out to be subtle. The semaphore permit acquired in the parent function must be **moved into the spawned task** — otherwise it's released when the parent returns and traffic flows in immediately. There's a Tokio idiom (`OwnedSemaphorePermit` + `forget()`) that handles this cleanly. There's also the question of *how* the fitted coefficients reach the workers once the probe finishes — that's the **lock-free `ArcSwap` handoff**, a wait-free pointer swap that lets readers and writers operate without coordination.

This page covers both: the traffic gate during the probe, and the coefficient handoff after.

## The concurrency gate during the probe window

Because the probe measures process-wide RSS deltas (`/proc/self/statm`), any concurrent `session.run()` call from real traffic pollutes the per-shape measurement. To prevent RSS contamination, the probe **holds all `cfg_workers` permits** for the duration of the probe + readiness window, blocking all incoming `/v1/embeddings*` requests at the semaphore. Requests queue rather than being rejected; `/health` still returns 200 during this window. The dense + sparse readiness checks and the `state.ready = true` flip both happen **inside** the probe task after the probe sweep completes.
...
`acquire_many_owned` returns a `tokio::sync::OwnedSemaphorePermit` independent of the source `Semaphore` lifetime, so it can be moved into the closure and is held for the full duration of the probe:
...
Two synchronization primitives make the post-probe coefficient handoff safe.
...
The `AtomicU8` ensures the status update is single-write, single-read consistent — no torn reads, no need for a lock, no need for `Ordering::SeqCst` in the hot path. We use `Ordering::Relaxed` for the read in `/health` and `Ordering::Release` for the write at the end of the probe task; `/health` is allowed to lag briefly behind the actual state, which is acceptable for a status field that's read by humans and load balancers.
...
The probe shouldn't hang in practice — it has timeouts on individual `session.run()` calls and the OOM-protection layers in [Probe shapes](06-probe-shapes.md) will skip pathologically large shapes before they're attempted. If a shape errors out, the probe logs and skips, never blocks. The whole sweep wraps in a Tokio timeout (configurable, defaults to 5 minutes) so even unforeseen hangs eventually trigger a fall-back-to-conservative path.

If you ever observe a `running` status that persists past 5 minutes:

- Check `/health` for `live_workers` — if any workers are dead, the pool is degraded
- Check container logs for `Probe: skipping shape` messages — a shape may be repeatedly hitting OOM guards
- Check `BGE_M3_MAX_SEQ_LENGTH` — if it's set higher than the model variant supports, the dynamic `(1, max_seq)` shape will hang on positional-embedding errors before timing out
...
## The choreography in plain English
...
2. **Workers load and prime.** Each worker measures pre-load RSS, downloads the model (cache hit on warm starts), creates the ORT session, runs a tiny `session.run((1, 8))` to prime the arena, measures post-load RSS, sends its delta to `main()`.
3. **`main()` aggregates deltas.** Median across workers becomes `model_rss_per_worker`. The workspace budget formula computes `max_workspace_bytes` from container memory, the worker count, and the safety factor.
4. **Probe-cache check.** If the fingerprint matches, jump to step 7.
5. **`main()` drains the semaphore.** Acquires all `cfg_workers - 1` remaining permits via `acquire_many_owned`, spawning the probe task with the permit `move`d in.
...
7. **Inside the probe task:** arena warm-up, sweep 7 shapes (each protected by 3 OOM-guard layers), fit OLS in normalized space, unscale, clamp, save cache, run dense + sparse readiness probes.
...
The four properties this design has, all together:
...
There's no alternative I'm aware of that gets all four. A serial design fails on (1). A naïve background design fails on (2). A "restart workers when probe finishes" design fails on (3). A "no status field" design fails on (4). Each piece of machinery is there because removing it breaks one of these properties.

## What's next

The next page walks through a real cold-start log trace from a 28 GB Fargate task, annotating each line with which page in this series it corresponds to.
