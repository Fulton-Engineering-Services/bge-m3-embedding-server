...
> A field guide for diagnosing probe state in production, forcing fresh measurements, and pinning explicit coefficients when needed. If you don't care how the probe works internally, this is the page to bookmark.

## Intuition

When something looks off in production — slower than expected throughput, a startup `WARN` about utilization, an unexplained OOM kill — the first place to look is `/health`. It exposes everything the probe knows about itself: the fitted coefficients, the budget formula's inputs, the probe lifecycle state, and the current memory headroom. Reading it correctly tells you in seconds whether the probe is healthy, whether it's running on conservative defaults, whether the container is over-subscribed, or whether a model variant is incompatible with the configured `MAX_SEQ_LENGTH`.

This page is structured as **diagnose first, fix second**. Each section starts with a symptom you might see, then walks through the diagnosis path, then recommends fixes. The fixes are env-var oriented because that's the only operator-facing knob — the probe doesn't have an admin API.
...
| `probe_status: running` for >5 min | Probe hung on a shape that can't run | `BGE_M3_MAX_SEQ_LENGTH` exceeds model variant's positional embedding limit | Lower `MAX_SEQ_LENGTH` or switch `BGE_M3_MODEL` to `fp32` |
...
| `a` looks like 16 384 and `b` looks like 8 | Conservative defaults are active (probe didn't run or failed) | `probe_status` field | If `cache_hit`, fine. If `failed`/`disabled`, see those rows. |
...
| Throughput unexpectedly low at short sequences | `a` clamped high, conservative `b` | Fitted `a` looks > 50 000? Probe captured a kernel-switch artifact | Force re-probe (`BGE_M3_DISABLE_PROBE_CACHE=1`); if persistent, file a bug with the cold-start log |
| `b` is `0` or extremely small | Fit corrupted by RSS noise | `model_rss_bytes_per_worker` should be ~1.1 GB; if it's 0, see that row | Force re-probe; investigate RSS reading |
...
Bypasses the cache without affecting other behavior. Use when validating a new deployment, after manual edits to the cache file, or when debugging probe behavior. The fresh fit is still written to the cache (overwriting the previous entry); subsequent starts can use the cache normally — set the env var only on the validation start.
...
All three must be set together — partial overrides are intentionally rejected (see `Config::from_env`). Use when reproducing a production incident locally with the same coefficients. Useful when the fitted values from a production trace need to be replayed in a controlled environment.

This is the only path that gives you direct control over `(a, b)`. The probe is bypassed; `probe_status` becomes `disabled`.

## Resizing workers

The most common production tuning is changing `BGE_M3_WORKERS`. The trade-off:

- **More workers → more parallelism, less per-worker budget.** Each worker gets `(available − N×model_rss − OS_HEADROOM) × safety / N`. Doubling N roughly halves the per-worker budget.
...
A rule of thumb that's worked well in production:

| Container memory | Recommended workers (fp16) | Per-worker workspace | Notes |
|------------------|----------------------------|---------------------|-------|
| 14 GB | 2 | ~3 GB | Comfortable for `MAX_SEQ=8192` |
| 28 GB | 4–5 | ~3 GB | Production sweet spot |
| 28 GB | 7 | ~1 GB | Throughput-tuned; tighter packing |
| 56 GB | 10–14 | ~3 GB | High-throughput tier |

Always re-probe after changing `BGE_M3_WORKERS` — although technically the workers count doesn't invalidate the cache (page [Cache §9](09-cache.md)), the *budget* changes, and you want to verify `utilization_pct` stays under 90%.
...
|----------|-------------------|
| `Memory detected available_bytes=... source=...` | Did cgroup detection work? `source=cgroup_v2` is normal in Fargate; `source=sysctl` means we're on macOS or cgroups failed |
| `Workspace budget computed ... utilization_pct=...` | Final budget formula output; `> 90%` triggers a startup `WARN` |
| `Probe cache fingerprint mismatch; will re-probe` | Cache was present but didn't match — what changed? (model, max_seq, arch, server_version) |
| `Probe: skipping shape (estimated to exceed rss_ceiling)` | OOM-protection layer in [Probe shapes §6](06-probe-shapes.md) — typically benign on small containers |
| `Probe shape failed; skipping batch=N seq=N error=...` | A shape errored at runtime; if `seq` matches `MAX_SEQ`, model variant is incompatible with that length |
...
| `Cost model: using conservative defaults` | Probe didn't run (disabled) or failed; conservative `(16384, 8)` are active |
...
A good post-deployment smoke test:
...
Any of these failing is a signal to investigate before declaring the deployment ready.

## When the probe is the wrong answer

Some operating modes don't benefit from the probe:

- **Local macOS dev** — RSS reads don't work, the probe falls back to defaults anyway. Set `BGE_M3_DISABLE_AUTO_BUDGET=1` to skip the probe machinery entirely and save a few seconds at startup.
- **CI smoke tests** — every probe run takes ~120 s. For tests that just verify the server boots and accepts requests, `BGE_M3_DISABLE_AUTO_BUDGET=1` is much faster.
...
|---------|---------|---------------|
| `BGE_M3_WORKERS` | `2` | Always — tune for your container size |
| `BGE_M3_MAX_SEQ_LENGTH` | `8192` | Only if your model variant doesn't support 8192 (lower it) |
| `BGE_M3_MODEL` | `fp16` | If on Apple Silicon (use `fp32`) or if probe shows positional-embedding errors at `MAX_SEQ` (try `fp32`) |
...
The last page in the series is the bibliography — sources for everything from OLS conditioning theory to ORT arena allocator behavior, for anyone wanting to dig deeper.
